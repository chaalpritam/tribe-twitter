...
struct AppKey {
    ...
    /// Restore an app key from a base64-encoded 32-byte seed.
    /// Throws if the input isn't valid base64 or the wrong length.
    static func restore(seedBase64: String) throws -> AppKey {
        ...
        guard raw.count == 32 else { throw AppKeyError.wrongLength(raw.count) }
...
enum AppKeyError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .wrongLength(let n):
            return "App key must be exactly 32 bytes; got \(n)."
        }
    }
}
